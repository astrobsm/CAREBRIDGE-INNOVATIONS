 ...
 * STANDALONE VERSION - Extracted for Integration into External Applications
 * 
 ...
 * - Consent & leaflet document generation
...
import type {
  ...
  SubstanceCategory,
} from '../types/substance-use.types';
...
export function calculatePhysicalDependenceScore(
  ...
  return { tolerance, withdrawalSymptoms, compulsiveUse, physicalCravings, totalScore };
}
...
export function calculatePsychologicalDependenceScore(
  ...
  return { emotionalReliance, copingMechanism, preoccupation, anxietyWithoutSubstance, totalScore };
}
...
export function calculateBehavioralDysfunctionScore(
  ...
  return { prioritizingSubstance, failedAttemptsToCut, timeSpentObtaining, givingUpActivities, totalScore };
}
...
export function calculateSocialImpairmentScore(
  ...
  return { occupationalImpact, relationshipImpact, financialImpact, legalIssues, totalScore };
}
...
export function calculateMedicalComplicationsScore(
  ...
  return { liverDysfunction, renalDysfunction, cardiacComplications, neurologicalComplications, infectiousComplications, psychiatricComorbidity, totalScore };
}
