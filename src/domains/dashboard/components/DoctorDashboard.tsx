    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      {showEarnings && myEarnings && (
        <motion.div
          ...
          className="bg-gradient-to-r from-emerald-500 to-teal-600 rounded-xl shadow-lg p-6 text-white"
        >
          <div className="flex items-center justify-between">
            <div>
          ...
          {Object.keys(myEarnings.byCategory).length > 0 && (
            <div className="mt-4 pt-4 border-t border-white/20">
              <p className="text-xs text-emerald-100 mb-2">Breakdown by Activity</p>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {Object.entries(myEarnings.byCategory).slice(0, 4).map(([category, amount]) => (
                  <div key={category} className="bg-white/10 rounded-lg p-2">
                    <p className="text-xs text-emerald-100 capitalize">{category.replace(/_/g, ' ')}</p>
