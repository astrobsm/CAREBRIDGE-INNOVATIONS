...
import {
  ...
  ENGINE_VERSION, type RunResult, type ClinicianAnalysis as SavedAnalysisSummary,
} from '../services/clinicianAssistantService';
import type { Severity } from '../services/engine/types';
import BedsideCalculators from '../components/BedsideCalculators';
import ScanReportsPanel from '../components/ScanReportsPanel';
import { buildFromPatientRecord } from '../services/patientBridge';
import { runForContext } from '../services/clinicianAssistantService';
...
interface PatientRow {
  // AstroHEALTH patient ids are UUID strings, not the numeric ids the original
    ...
    (async () => {
      ...
      try {
        const [p, r] = await Promise.all([
          // Read straight from the local mirror so the picker works offline.
          db.patients.toArray().catch(() => []),
          listRecent(25).catch(() => []),
        ]);
        const rows: PatientRow[] = p
          .filter(x => x.isActive !== false)
          .map(x => ({
            id: x.id,
            first_name: x.firstName,
            last_name: x.lastName,
            hospital_number: x.hospitalNumber,
          }));
        setPatients(rows);
        setRecent(r);
      } finally {
                ...
                {recent.map(a => (
                  <div key={a.id} className="bg-white border rounded-lg p-2.5">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className={`px-1.5 py-0.5 rounded text-xs border ${tone(a.overallSeverity)}`}>
                        {a.overallSeverity}
                      </span>
                    </div>
                    <p className="text-xs text-gray-500 mt-0.5">
                      {new Date(a.analysedAt).toLocaleString()}
                    </p>
                  </div>
                ))}
              </div>
              ...
              <div className="divide-y">
                {history.map(h => (
                  <div key={h.id} className="py-2 flex items-center justify-between gap-2">
                    <span className="text-xs text-gray-600">{new Date(h.analysedAt).toLocaleString()}</span>
                    <span className={`px-1.5 py-0.5 rounded text-xs border ${tone(h.overallSeverity)}`}>
                      {h.overallSeverity}
                    </span>
                  </div>
                ))}
